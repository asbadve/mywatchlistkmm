...
class PrivacyConsentRepositoryImplTest {
    // Mirrors NotificationSettingsRepositoryImplTest's FakeSettings - boolean-only is all this
    // repository needs.
    ...
    @Test
    fun testDefaultsToNotAccepted() {
        val repository = PrivacyConsentRepositoryImpl(FakeSettings())

        assertFalse(repository.hasAcceptedPrivacyPolicy())
    }

    @Test
    fun testMarkPrivacyPolicyAcceptedPersists() {
        val settings = FakeSettings()
        val repository = PrivacyConsentRepositoryImpl(settings)

        repository.markPrivacyPolicyAccepted()

        assertTrue(repository.hasAcceptedPrivacyPolicy())
        assertTrue(settings.getBoolean(PrivacyConsentConstant.KEY_PRIVACY_POLICY_ACCEPTED, false))
    }

    @Test
    fun testAcceptancePersistsAcrossRepositoryInstances() {
        val settings = FakeSettings()
        PrivacyConsentRepositoryImpl(settings).markPrivacyPolicyAccepted()

        val secondInstance = PrivacyConsentRepositoryImpl(settings)

        assertTrue(secondInstance.hasAcceptedPrivacyPolicy())
    }
}
