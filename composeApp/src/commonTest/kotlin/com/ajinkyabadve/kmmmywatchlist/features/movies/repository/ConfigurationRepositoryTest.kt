...
class ConfigurationRepositoryTest {
    ...
    private class FakeConfigurationRepository(
        ...
        override suspend fun getConfiguration(): ImagesConfig {
            ...
            settings.putString(
                ...
                "{\"base_url\":\"http://mock/\",\"secure_base_url\":\"https://mock/\",\"backdrop_sizes\":[],\"logo_sizes\":[],\"poster_sizes\":[],\"profile_sizes\":[],\"still_sizes\":[]}",
            )
        ...
        private fun getFallbackConfig(cachedJson: String): ImagesConfig = ConfigurationConstants.defaultImagesConfig
    }

    @Test
    fun testGetConfigurationReturnsCachedWhenFresh() =
    ...
    @Test
    fun testGetConfigurationFetchesNetworkWhenCacheExpired() =
        kotlinx.coroutines.test.runTest {
            val settings = FakeSettings()
            val repository = FakeConfigurationRepository(settings)
            ...
            settings.putLong(
                ConfigurationConstants.KEY_TIMESTAMP,
    ...
    @Test
    fun testGetConfigurationClearsCorruptedCache() =
        kotlinx.coroutines.test.runTest {
            val settings = FakeSettings()
            val repository = FakeConfigurationRepository(settings)
    ...
    @Test
    fun testGetConfigurationFallbackOnNetworkError() =
        kotlinx.coroutines.test.runTest {
            val settings = FakeSettings()
            val repository = FakeConfigurationRepository(settings)
            repository.shouldFailNetwork = true
            repository.exceptionToThrow =
                io.ktor.client.plugins
                    .HttpRequestTimeoutException("url", 1000L)
