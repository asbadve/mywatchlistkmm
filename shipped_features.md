...
The completed half of [`future_features_checklist.md`](future_features_checklist.md), moved out on 2026-09-24 when that file reached ~1,660
lines and roughly half of it was history. Nothing here is a backlog item: every section below
shipped (or is explicitly on hold). It is kept in full rather than deleted because the reasoning,
the rejected alternatives and the documented dead ends are the parts worth not re-deriving -
several of them were load-bearing when the newer items were written.

Open work lives in [`future_features_checklist.md`](future_features_checklist.md). The three unchecked follow-ups that remain inside these
shipped items are indexed there too, so they are not lost behind a "DONE" marker.
...
- [x] **Data Layer**: `TrackedMediaRepository`/`SqliteTrackedMediaRepositoryImpl` wrap the
  ...
  keeps the local table in step with `AccountMediaRepository` ([item 6](#6-account-favorites--watchlist-replacing-my-fav-placeholder--done-2026-08-16))
...
top of it. **Depends on [#6](#6-account-favorites--watchlist-replacing-my-fav-placeholder--done-2026-08-16)**: needs
...
[item 2](#2-local-sqlite-database-for-favoriteswatchlist-local-notification-data-source--done)**: the
